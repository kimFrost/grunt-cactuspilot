...
---------------------------------------------------------------*/
	plugin = {
		...
		getBlockElements: function(string, blockName) {
			var elements = [];
			console.log(" \n --- Elements Generation --- \n ");
			var lineArray = string.toString().split(/\n/);
			for (var i=0;i<lineArray.length;i++) {
		...
		enrichWithStates: function(string, list) {
			// Loop through list and find the items states
			console.log(" \n --- Enrich Generation --- \n ");
			for (var i=0;i<list.length;i++) {
				var elem = list[i];
				console.log(elem);
				// Create a states array on the element
				elem.states = [];
				// Find all states name elem.value + "--" and append them to elem object under states
				var lineArray = string.toString().split(/\n/);
				for (var i=0;i<lineArray.length;i++) {
					var line = lineArray[i];
					if (line.indexOf(elem.value + "--") != -1) {
						console.log(line);
	...
	grunt.registerMultiTask('cactuspilot', 'Sass BEM styleguide generation', function() {
		...
		this.files.forEach(function(f) {
			...
			for (var i=1;i<modules.length;i++) {
				...
				var modulesElements = plugin.getBlockElements(src, modulename[0].value);
				modulesElements.push(modulename[0]);
				var blockAndElements = modulesElements;

				var blockAndElementsEnriched = plugin.enrichWithStates(src, blockAndElements);
